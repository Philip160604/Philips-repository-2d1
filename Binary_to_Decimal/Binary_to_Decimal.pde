...
class Bit {             // Bit object class
  ...
  // circle blev ændret til sqaure og postitionen blev ændret fra "position" til "position - 25"
  void display() {
    fill(colour);
    square(width/9 * position - 25, 60, 50);
  }
  ...
  void updateMouse() {
    if (onCircle(width/9 * position, 60, 50)) {
      switch(colour) {
        case (55):
          colour = (255);
